...
sealed class SmartTapStatus(
    ...
    class Unknown(val status: UShort) : Error(
        sw1 = (status.toUInt() shr 8).toUByte(),
        sw2 = status.toUByte(),
    )
    ...
    companion object {
        private fun hex(value: UByte): String = "0x${value.toString(16).padStart(2, '0').uppercase()}"

        val entries: List<SmartTapStatus> = listOf(
            Ok,
            OkNoPayload,
            OkPreSignedAuth,
            OkMorePayload,
        ...
        fun from(sw1: UByte, sw2: UByte): SmartTapStatus {
            bySW[(sw1.toUInt() shl 8) or sw2.toUInt()]?.let { return it }
        ...
        fun from(response: Iso7816Response): SmartTapStatus =
            from(response.sw1, response.sw2)
    }
}
